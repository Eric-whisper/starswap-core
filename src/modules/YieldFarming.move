...
module YieldFarming {
    ...
    public fun modify_parameter<PoolType: store, RewardTokenT: store, AssetT: store>(
        ...
        update_index<PoolType, AssetT>(farming_asset, now_seconds);
    ...
    public fun stake<PoolType: store, RewardTokenT: store, AssetT: store>(
        ...
        if (farming_asset.asset_total_weight <= 0) {
            move_to(account, Stake<PoolType, AssetT>{
                ...
                gain: farming_asset.release_per_second * (time_period as u128),
            });
            ...
            let new_harvest_index = update_index<PoolType, AssetT>(farming_asset, now_seconds);
            move_to(account, Stake<PoolType, AssetT>{
                asset,
                asset_weight,
    ...
    public fun unstake<PoolType: store, RewardTokenT: store, AssetT: store>(account: &signer, broker: address) 
        ...
        let period_gain = calculate_withdraw_amount(farming_asset.harvest_index, last_harvest_index, asset_weight);
        ...
        update_index<PoolType, AssetT>(farming_asset, Timestamp::now_seconds());
    ...
    public fun harvest<PoolType: store,
        ...
        stake.gain = stake.gain - withdraw_amount;
        stake.last_harvest_index = update_index<PoolType, AssetT>(farming_asset, Timestamp::now_seconds());
    ...
    public fun query_gov_token_amount<PoolType: store,
        ...
        let new_gain = calculate_withdraw_amount(
            farming_asset.harvest_index, stake.last_harvest_index, stake.asset_weight);
        let new_index = update_index<PoolType, AssetT>(farming_asset, Timestamp::now_seconds());
        
        stake.gain = stake.gain + new_gain;
        stake.last_harvest_index = new_index;
        stake.gain
    }
    ...
    fun update_index<PoolType, AssetT>(farming_asset: &mut FarmingAsset<PoolType, AssetT>, now_seconds: u64) : u128 {
        ...
        if (farming_asset.asset_total_weight <= 0) {
            ...
            let time_period = now_seconds - farming_asset.last_update_timestamp;
            farming_asset.harvest_index = farming_asset.harvest_index + (farming_asset.release_per_second * ((time_period as u128)));
        } else {
    ...
    public fun calculate_harvest_index(harvest_index: u128,
        ...
        let time_period = last_update_timestamp - now_seconds;
