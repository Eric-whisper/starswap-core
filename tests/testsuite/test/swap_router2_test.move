...
script {
    ...
    fun register_token_pair(signer: signer) {
        ...
        TokenSwapRouter::register_swap_pair<ETH, BTC>(&signer);
        assert(TokenSwapRouter::swap_pair_exists<ETH, BTC>(), 115);
    }
}
...
address alice = {{alice}};
script {
    use 0x598b8cbfd4536ecbe88aa1cfaffa7a62::TokenSwapRouter2;
    use alice::SwapTestHelper::{ETH, DOT, BTC};

    fun swap_pair_not_exist(signer: signer) {
        let amount_x_in = 200;
        let amount_y_out_min = 500;
        TokenSwapRouter2::swap_exact_token_for_token<ETH, BTC, DOT>(&signer, amount_x_in, amount_y_out_min);
    }
}
...
script {
    ...
    fun swap_exact_token_for_token(signer: signer) {
        ...
        let token_balance = SwapTestHelper::get_safe_balance<USDT>(Signer::address_of(&signer));
        assert(token_balance == 0, 201);

        let (middle_out, expected_token_balance) = TokenSwapRouter2::get_amount_out<STC, ETH, USDT>(amount_x_in); 
        TokenSwapRouter2::swap_exact_token_for_token<STC, ETH, USDT>(&signer, amount_x_in, amount_y_out_min);
        // TokenSwapRouter::swap_exact_token_for_token<STC, ETH>(&signer, amount_x_in, middle_out);
        // TokenSwapRouter::swap_exact_token_for_token<ETH, USDT>(&signer, middle_out, amount_y_out_min);
        let token_balance = SwapTestHelper::get_safe_balance<USDT>(Signer::address_of(&signer));
...
script {
    ...
    fun swap_token_for_exact_token(signer: signer) {
        ...
        let (middle_in, x_in) = TokenSwapRouter2::get_amount_in<ETH, USDT, DAI>(amount_y_out); 
        TokenSwapRouter2::swap_token_for_exact_token<ETH, USDT, DAI>(&signer, amount_x_in_max, amount_y_out);

        // TokenSwapRouter::swap_token_for_exact_token<ETH, USDT>(&signer, amount_x_in_max, middle_in);
        // TokenSwapRouter::swap_token_for_exact_token<USDT, DAI>(&signer, middle_in, amount_y_out);
