...
script {
    ...
    fun add_liquidity(signer: &signer) {
        TokenSwapRouter::add_liquidity<STC::STC, Token1::Token1>(signer, 10000, 10000 * 10000, 0, 0);
        let total_liquidity = TokenSwapRouter::total_liquidity<STC::STC, Token1::Token1>();
...
script {
    ...
    fun remove_liquidity(signer: &signer) {
        TokenSwapRouter::remove_liquidity<STC::STC, Token1::Token1>(signer, 10000, 0, 0);
        ...
        let (stc_reserve, token_reserve) = TokenSwapRouter::get_reserves<STC::STC, Token1::Token1>();
...
script {
    ...
    fun swap_exact_token_for_token(signer: &signer) {
        let (stc_reserve, token_reserve) = TokenSwapRouter::get_reserves<STC::STC, Token1::Token1>();
        TokenSwapRouter::swap_exact_token_for_token<STC::STC, Token1::Token1>(signer, 1000, 0);
        let token_balance = Account::balance<Token1::Token1>(Signer::address_of(signer));
...
script {
    ...
    fun swap_token_for_exact_token(signer: &signer) {
        ...
        let (stc_reserve, token_reserve) = TokenSwapRouter::get_reserves<STC::STC, Token1::Token1>();
        TokenSwapRouter::swap_token_for_exact_token<STC::STC, Token1::Token1>(signer, 30, 100000);
